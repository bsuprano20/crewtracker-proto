...
with st.form("new_entry"):
    ...
    job_id = st.selectbox("Job", JOBS["id"],
                         format_func=lambda x: JOBS.loc[JOBS.id==x, "name"].iloc[0])
    cc_id = st.selectbox("Cost Code", COST_CODES["id"],
    ...
    if st.form_submit_button("Save Entry"):
        ENTRIES.loc[len(ENTRIES)] = {
            "date": d, "job_id": job_id,
            "cost_code_id": cc_id, "hours": hrs,
...
sel = st.selectbox("Select Job", JOBS["id"],
                   format_func=lambda x: JOBS.loc[JOBS.id==x, "name"].iloc[0])
df = ENTRIES[ENTRIES.job_id == sel].copy()
if not df.empty:
    df = df.merge(COST_CODES[["id","pmph_goal"]],
                  left_on="cost_code_id", right_on="id")
    df["estimated"] = df["hours"] * df["pmph_goal"]
    df["variance"]  = df["units_installed"] - df["estimated"]
    st.dataframe(df[["date","cost_code_id","hours",
                     "units_installed","estimated","variance"]])
    st.line_chart(df.set_index("date")[["units_installed","estimated","variance"]])
else:
